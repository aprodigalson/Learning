...
import game2048.const as c
import game2048.logic as logic
...
class GameGrid(Frame):
    def __init__(self):
        ...
        self.commands = {
            c.KEY_UP: logic.up, c.KEY_DOWN: logic.down,
    ...
    def key_down(self, event):
        ...
        if key == c.KEY_BACK and len(self.history_matrixs) > 1:
            ...
            # main step
            self.matrix, done = self.next_move(key)
            if done:
                self.matrix = logic.add_two(self.matrix)
    ...
    def next_move(self, key):
        return self.commands[key](self.matrix)
